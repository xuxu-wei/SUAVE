...
def _categorical_from_codes(
    ...
    *,
    ...
    if categories is None:
        ...
        categories_list = list(range(max_code + 1)) if max_code >= 0 else []
    else:
...
def decoder_outputs_to_frame(
    ...
    for column in schema.feature_names:
        ...
        if spec.type in {"real", "pos"}:
            ...
            data[column] = _categorical_from_codes(codes, categories)
            ...
            categories = norm_stats.get(column, {}).get("categories")
            data[column] = _categorical_from_codes(codes, categories, ordered=True)
        else:
