...
class SUAVE:
    ...
    def fit(
        ...
        *,
        ...
        monitor_save_path: str | Path | None = None,
        monitor_save_formats: str | Iterable[str] = "jpg",
    ) -> "SUAVE":
        ...
        monitor_save_path : str or pathlib.Path, optional
            Directory used to persist training-monitor snapshots when
            :paramref:`plot_monitor` is ``True``. The directory must already
            exist; otherwise, snapshots are skipped.
        monitor_save_formats : str or Iterable[str], default "jpg"
            Image formats used when persisting monitor snapshots. Accepts a
            single extension (``"jpg"`` by default) or an iterable of
            extensions such as ``("jpg", "png")``. Snapshots are written at
            360 DPI for each requested format when :paramref:`monitor_save_path`
            is valid.
        ...
        if plot_monitor:
            ...
            monitor = _TrainingPlotMonitor(
                self.behaviour,
                save_path=monitor_save_path,
                save_formats=monitor_save_formats,
            )
        epoch_cursor = 0
        for name, value in {
