...
benchmark_curve_paths: List[Tuple[str, Path]] = []
...
for dataset_name in benchmark_datasets:
    ...
    figure_paths = plot_benchmark_curves(
        dataset_name,
    ...
    if figure_paths is not None:
        for curve_type, path in figure_paths.items():
            benchmark_curve_paths.append((curve_type, path))
...
#
# Purpose: quantify latent-feature-target relationships via correlation analyses
# and project cohort embeddings for qualitative review. Outputs reside in the
# dedicated interpretation directory so reviewers can trace back to model
# explainability artefacts before inspecting downstream transfer results.
...
for curve_type, figure_path in benchmark_curve_paths:
    summary_lines.append(
        f"- Benchmark {curve_type} curve: {figure_path.relative_to(OUTPUT_DIR)}"
    )
summary_lines.append("")
