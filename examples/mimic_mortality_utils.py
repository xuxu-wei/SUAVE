...
#: Preferred formats for persisting analysis figures.
FIG_SAVE_FORMATS: tuple[str, ...] = ("jpg", "png", "svg", "pdf")
...
def _save_figure_multiformat(
    ...
    *,
    ...
    """Persist ``figure`` using the configured :data:`FIG_SAVE_FORMATS`."""

    save_kwargs: Dict[str, object] = {"dpi": dpi}
    if use_tight_layout:
        save_kwargs["bbox_inches"] = "tight"
    for suffix in FIG_SAVE_FORMATS:
        figure.savefig(base_path.with_suffix(f".{suffix}"), **save_kwargs)
    ...
    *,
    ...
    if (not best_info or not best_params) and storage and study_name:
        ...
        if optuna is not None:  # pragma: no cover - optuna available in examples env
            handled_exceptions: tuple[type[Exception], ...] = (KeyError,)
            exceptions_module = getattr(optuna, "exceptions", None)
            if exceptions_module is not None:
                for attribute in (
                    ...
                    candidate = getattr(exceptions_module, attribute, None)
                    if isinstance(candidate, type) and issubclass(candidate, Exception):
                        handled_exceptions = handled_exceptions + (candidate,)
            try:
                study = optuna.load_study(study_name=study_name, storage=storage)
            except handled_exceptions as error:  # type: ignore[misc]
                print(
                    f"Optuna study '{study_name}' not found in storage '{storage}'; "
                    f"skipping remote lookup ({error})."
                )
            else:
                feasible_trials = [
                ...
                if feasible_trials:
                    ...
                    best_info = {
                        ...
                        "tstr_metrics": best_trial.user_attrs.get(
                            "tstr_metrics", {}
                        ),
                        "trtr_metrics": best_trial.user_attrs.get(
                            "trtr_metrics", {}
                        ),
...
def run_manual_override_training(
    *,
    ...
    monitor_dir = model_dir / "monitor_plot"
    monitor_dir.mkdir(parents=True, exist_ok=True)

    fit_kwargs = resolve_suave_fit_kwargs(merged_params)
    fit_kwargs.setdefault("monitor_save_path", monitor_dir)
    fit_kwargs.setdefault("monitor_save_formats", FIG_SAVE_FORMATS)

    model.fit(
        X_train,
        y_train,
        plot_monitor=is_interactive_session(),
        **fit_kwargs,
    )
...
def resolve_suave_fit_kwargs(params: Mapping[str, object]) -> Dict[str, object]:
    ...
    def _coerce_numeric(
        key: str, default: object, caster: Callable[[object], object]
    ) -> Optional[object]:
        raw_value = params.get(key, sentinel)
        ...
        return cast(object, caster(raw_value))

    return {
        "warmup_epochs": _coerce_numeric("warmup_epochs", 3, int),
        "kl_warmup_epochs": _coerce_numeric("kl_warmup_epochs", 0, int),
        "head_epochs": _coerce_numeric("head_epochs", 2, int),
        "finetune_epochs": _coerce_numeric("finetune_epochs", 2, int),
        "joint_decoder_lr_scale": _coerce_numeric(
            "joint_decoder_lr_scale", 0.1, float
        ),
        "early_stop_patience": _coerce_numeric("early_stop_patience", 10, int),
    }
...
def build_suave_model(
    ...
    *,
    ...
    hidden_raw = params.get("hidden_dims", sentinel)
    if hidden_raw is sentinel:
        hidden_dims: Optional[Tuple[int, ...]] = HIDDEN_DIMENSION_OPTIONS["medium"]
    elif hidden_raw is None:
        hidden_dims = None
    elif isinstance(hidden_raw, str):
        hidden_dims = HIDDEN_DIMENSION_OPTIONS.get(
            hidden_raw, HIDDEN_DIMENSION_OPTIONS["medium"]
        )
    else:
        hidden_dims = tuple(int(dim) for dim in cast(Iterable[int], hidden_raw))

    head_hidden_raw = params.get("head_hidden_dims", sentinel)
    if head_hidden_raw is sentinel:
        head_hidden_dims = HEAD_HIDDEN_DIMENSION_OPTIONS["medium"]
    elif head_hidden_raw is None:
        head_hidden_dims = HEAD_HIDDEN_DIMENSION_OPTIONS["medium"]
    elif isinstance(head_hidden_raw, str):
        head_hidden_dims = HEAD_HIDDEN_DIMENSION_OPTIONS.get(
            head_hidden_raw, HEAD_HIDDEN_DIMENSION_OPTIONS["medium"]
        )
    else:
        head_hidden_dims = tuple(
            int(dim) for dim in cast(Iterable[int], head_hidden_raw)
        )
    ...
    latent_dim = _coerce_optional_numeric("latent_dim", 16, int)
    dropout = _coerce_optional_numeric("dropout", 0.1, float)
    learning_rate = _coerce_optional_numeric("learning_rate", 1e-3, float)
    batch_size = _coerce_optional_numeric("batch_size", 256, int)
    beta = _coerce_optional_numeric("beta", 1.5, float)
    ...
    if latent_dim is not None:
        suave_kwargs["latent_dim"] = latent_dim
    if dropout is not None:
        suave_kwargs["dropout"] = dropout
    if learning_rate is not None:
        suave_kwargs["learning_rate"] = learning_rate
    if batch_size is not None:
        suave_kwargs["batch_size"] = batch_size
    if beta is not None:
        suave_kwargs["beta"] = beta

    return SUAVE(**suave_kwargs)
