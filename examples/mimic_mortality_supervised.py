    ...
    *,
    ...
    def objective(trial: "optuna.trial.Trial") -> float:
        latent_dim = trial.suggest_categorical(
            "latent_dim", [8, 16, 24, 32, 48, 64, 96, 128]
        )
        ...
        warmup_epochs = trial.suggest_int("warmup_epochs", 1, 8)
        head_epochs = trial.suggest_int("head_epochs", 1, 6)
...
def main() -> None:
    ...
    for target in TARGET_COLUMNS:
        ...
        membership_records.append(
            {
                "target": target,
                "auc": membership.get("attack_auc", float("nan")),
                "best_threshold": membership.get(
                    "attack_best_threshold", float("nan")
                ),
                "best_accuracy": membership.get("attack_best_accuracy", float("nan")),
                "majority_accuracy": membership.get(
                    "attack_majority_class_accuracy", float("nan")
                ),
            }
        )
    ...
    if tstr_results is not None:
        ...
        summary_lines.append("| --- | --- | --- | --- | --- | --- |")
        for row in tstr_results.itertuples(index=False):
            summary_lines.append(
                "| {setting} | {acc} | {auc} | {auprc} | {brier} | {ece} |".format(
                    setting=row.setting,
                    acc=format_float(row.accuracy),
                    auc=format_float(getattr(row, "auroc", np.nan)),
                    auprc=format_float(row.auprc),
                    brier=format_float(row.brier),
                    ece=format_float(row.ece),
                )
            )
        summary_lines.append("")
    ...
    if not membership_df.empty:
        ...
        summary_lines.append(
            "| Target | AUC | Best accuracy | Threshold | Majority baseline |"
        )
        summary_lines.append("| --- | --- | --- | --- | --- |")
        for row in membership_df.itertuples(index=False):
            summary_lines.append(
                "| {target} | {auc} | {accuracy} | {threshold} | {majority} |".format(
                    target=row.target,
                    auc=format_float(row.auc),
                    accuracy=format_float(row.best_accuracy),
                    threshold=format_float(row.best_threshold),
                    majority=format_float(row.majority_accuracy),
                )
            )
        summary_lines.append(
