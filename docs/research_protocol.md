...
| 8. 分类/校准评估与不确定性量化（Bootstrap） | 校准曲线 | 图像 | 展示 `plot_calibration_curves` 生成的各数据集校准关系 | 经过校准的预测概率与真实标签（`probability_map`、`label_map`） | `examples/research_outputs_supervised/01_data_and_schema/evaluation_datasets_in_hospital_mortality.joblib`<br>`examples/research_outputs_supervised/05_calibration_uncertainty/isotonic_calibrator_in_hospital_mortality.joblib` | <pre><code class="language-python">from pathlib import Path
import joblib
import numpy as np
