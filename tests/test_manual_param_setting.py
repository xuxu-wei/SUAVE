...
from suave import Schema, SUAVE
...
def test_resolve_suave_fit_kwargs_handles_optional_entries(module_path: str) -> None:
    """Optional numeric overrides should propagate ``None`` cleanly."""
    ...
    params = {
        ...
        "kl_warmup_epochs": " ",
        "head_epochs": "3",
        "finetune_epochs": np.int_(4),
        "joint_decoder_lr_scale": "",
        "early_stop_patience": None,
    }

    fit_kwargs = module.resolve_suave_fit_kwargs(params)
    assert fit_kwargs["warmup_epochs"] is None
    assert fit_kwargs["kl_warmup_epochs"] is None
    assert fit_kwargs["head_epochs"] == 3
    assert fit_kwargs["finetune_epochs"] == 4
    assert fit_kwargs["joint_decoder_lr_scale"] is None
    assert fit_kwargs["early_stop_patience"] is None
...
def test_build_suave_model_handles_optional_overrides(module_path: str) -> None:
    """Building SUAVE should allow ``None``/blank manual overrides."""

    module = importlib.import_module(module_path)
    schema = Schema({"age": {"type": "real"}})
    overrides = {
        "latent_dim": "",
        "dropout": None,
        "learning_rate": "",
        ...
        "beta": "",
        "hidden_dims": None,
        "head_hidden_dims": "medium",
    }

    model = module.build_suave_model(overrides, schema, random_state=0)
    assert isinstance(model, SUAVE)
    assert model.latent_dim == 32
    assert model.dropout == pytest.approx(0.1)
    assert model.batch_size == 128
    assert model.beta == pytest.approx(1.5)
...
def test_manual_defaults_load_without_type_error(
    tmp_path: Path, module_path: str
) -> None:
    """Manual defaults should feed into SUAVE helpers without errors."""
    ...
    manual_dir = directories["suave_model"]
    manual_config = dict(module.INTERACTIVE_MANUAL_TUNING)

    overrides = module.load_manual_tuning_overrides(manual_config, manual_dir)
    fit_kwargs = module.resolve_suave_fit_kwargs(overrides)
    assert fit_kwargs["warmup_epochs"] is None
    assert fit_kwargs["kl_warmup_epochs"] is None
...
def test_run_manual_override_training_history_flag(
    ...
    captured_fit_kwargs: List[Dict[str, Any]] = []
    ...
    class _DummyModel:
        ...
        def fit(self, *_args: object, **_kwargs: object) -> None:  # pragma: no cover - stub
            captured_fit_kwargs.append(dict(_kwargs))
    ...
    monkeypatch.setattr(module, "resolve_suave_fit_kwargs", lambda params: {"stub": "value"})
    ...
    assert captured_fit_kwargs[0]["stub"] == "value"
    assert "monitor_save_path" in captured_fit_kwargs[0]
    assert "monitor_save_formats" in captured_fit_kwargs[0]
    assert captured_fit_kwargs[0]["monitor_save_path"].name == "monitor_plot"
    assert captured_fit_kwargs[0]["monitor_save_formats"] == module.FIG_SAVE_FORMATS
    assert (model_dir / "monitor_plot").is_dir()
    ...
    assert captured_fit_kwargs[1]["stub"] == "value"
    assert captured_fit_kwargs[1]["monitor_save_path"].name == "monitor_plot"
    assert captured_fit_kwargs[1]["monitor_save_formats"] == module.FIG_SAVE_FORMATS
...
def test_load_optuna_results_handles_missing_remote_study(
    tmp_path: Path, module_path: str, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    """Optuna absence in storage should not raise and should emit a notice."""

    module = importlib.import_module(module_path)
    output_dir = tmp_path / "optuna_cache"
    output_dir.mkdir(parents=True, exist_ok=True)

    study_prefix = "demo"
    storage = "sqlite:///example.db"

    class _StubStudyNotFound(Exception):
        """Stub exception mirroring Optuna's missing-study errors."""

    optuna_stub = types.ModuleType("optuna")
    optuna_stub.exceptions = types.SimpleNamespace(StudyNotFound=_StubStudyNotFound)

    def _raise_missing_study(*_: object, **__: object) -> None:
        raise KeyError("missing study")

    optuna_stub.load_study = _raise_missing_study  # type: ignore[attr-defined]
    monkeypatch.setitem(sys.modules, "optuna", optuna_stub)

    best_info, best_params = module.load_optuna_results(
        output_dir,
        "mortality",
    ...
    assert study_prefix in captured.out
    assert "missing study" in captured.out
